// Program intended to demo the function of interrupt service routines and Optical Sensors
// User can command Forward or Reverse and then the rover will rotate wheels in tandem with 
// each other
...
volatile int lwheelPulse;
volatile int rwheelPulse;
int LWsensor;
int RWsensor;
...
int speed = 250; // Modify this value to set speed of robot, pulse count functions poorly at high speed
int speedUp;
int slowDown;
...
void interruptSetUp()
{
  attachInterrupt(0, lwheelSensor, CHANGE);
  attachInterrupt(1, rwheelSensor, CHANGE);
}
